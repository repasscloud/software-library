...
while ($appCategory.Length -eq 0) {
    [Int16]$appCategory=Read-Host -Prompt "App categories:`r`n  1 - browser`r`n  2 - business`r`n  3 - entertainment`r`n  4 - graphic & design`r`n  5 - photo`r`n  6 - social`r`n  7 - productivity`r`n  8 - games`r`n  9 - microsoft`r`nMake selection"
    switch ($appCategory) {
        1 {
...
while ($architecture.Length -eq 0) {
    ...
    [Int16]$architecture=Read-Host -Prompt "Select architecture`r`n  1 x86`r`n  2 x64`r`n  3 x86_x64`r`nMake selection"
    switch ($architecture) {
        1 {
...
switch ($archtype) {
    2 {
        ...
        $WebClient.DownloadFile($urlx64, $Hashfile)

        # This command will get the sha256 hash
        $Hash=Get-FileHash $hashfile
        [String]$json21='                "Sha256": "' + $Hash + '",'
        $OFS
    ...
    1 {
        ...
        $WebClient=New-Object System.Net.WebClient
        $WebClient.DownloadFile($urlx32, $Hashfile)

        # This command will get the sha256 hash
        $Hash=Get-FileHash $hashfile
        [String]$json28='                "Sha256": "' + $Hash + '",'
        $OFS
        write-host "32-bit file downloaded. Please Fill out required fields."
        [String]$json19='        "Url": "",'
    ...
    3 {
        ...
        if ($absoluteURL32 -notlike $url32) {
            $urlx32=$absoluteURL32
            [String]$json19='                "Url": "' + $url32 + '",'
            [String]$json20='                "FollowUrl": "' + $absoluteURL32 + '",'
        } else {
            $urlx32=$url32
            [String]$json19='                "Url": "' + $url32 + '",'
            [String]$json20='                "FollowUrl": "' + $url32 + '",'
        }
...
# Get application and publisher
$id=$null
while ($id -notmatch '.' -and $id.Length -lt 3) {
    $id=Read-Host 'Enter ID match please. For example: "<Publisher>.<AppName>"'
}
$appName=$id.Split('.')[1]
...
while ($Copyright.Length -eq 0) {
    $Copyright=Read-Host -Prompt 'Enter the License, For example: Copyright (c) Microsoft Corporation'
}
...
while ($License.Length -eq 0) {
    $License=Read-Host -Prompt 'Enter the License, For example: MIT, BSD, GNU, Proprietary'
}
...
<#
$outFile="../${publisher}/${appName}/${version}.json"
