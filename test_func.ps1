...
function Invoke-CreateCManiCore {
    ...
    begin {
        ...
        # Set Temp directory variable to $dir_tmp by OS selection, with backwards compatibility for Windows PS5.1
        if ($IsWindows -or $ENV:OS) {
            Write-Host -ForegroundColor Red "This is Windows!"
            [String]$dir_tmp=$Env:TEMP
        } else {
            if ($IsLinux) {
                Write-Host "Linux"
        ...
        Get-ChildItem -Path $dir_Scripts -Filter "*.ps1" -Recurse | ForEach-Object { Write-Host -ForegroundColor Green "Importing file: " -NoNewLine; Write-Host $_.FullName }

        # Create temporary file
        [String]$file_tmp=[System.IO.Path]::Combine($dir_tmp,$([System.GUID]::NewGUID().Guid)+'.json')
        Write-Host -ForegroundColor Yellow "`$file_tmp is: " -NoNewLine
        Write-Host $file_tmp

        # Manifest Copyright Notice
        [String]$RePassCloudManifestCopyrightNotice=@"
Copyright © 2020 RePassCloud.com
    ...
    process {
        ...
        if ('x64' -in $Arch) {
            ...
            foreach($_lang in $Languages) {
                ...
                $maniDict.Id.Installers.x64.$_lang.FollowURI='put-follow-uri-function-result-here'
                $maniDict.Id.Installers.x64.$_lang.Sha256='put-sha256-func-result-here'
                $maniDict.Id.Installers.x64.$_lang.Sha512='put-sha256-func-result-here'
...
Invoke-CreateCManiCore -Category browser `
                       -Publisher 'nobody is home_' `
                       -Name 'there is more than' `
                       -Version '21.0.22.36' `
                       -CopyrightNotice $CopyNoticeGoesHere `
                       -License 'BSD-2-Clause' `
                       -LicenseURI https://www.google.com `
                       -Tags @('cats','dogs','tags') `
                       -Description 'brief descripiton of application' `
                       -Homepage https://www.yahoo.com `
                       -Arch @('x64','x86') `
                       -Languages @('en-US','en-AU') `
                       -NuspecFile 'https://www.google.com' `
                       -Depends @('bob.smith','mary.jane','eating.f1') `
                       -InstallURI_x64 @('https://raw.githubusercontent.com/repasscloud/software-library/master/CHANGELOG.md','https://raw.githubusercontent.com/repasscloud/software-library/master/README.md') `
                       -MsiExe_x64 msi `
                       -MsiExe_x86 'exe' `
                       -UninstallExe_x64 'UninstallExe_x64.msi' `
                       -UninstallExe_x86 'UninstallExe_x86.exe' `
                       -UninstallArgs_x64 '/Uninstallargs_x64' `
                       -UninstallArgs_x86 '/Uninstallargs_x86' `
                       -UpdateURI_x64 'https://mcdonalds.com.au/' `
                       -UpdateURI_x86 @('https://mcdonalds.com.au/','https://www.coles.com.au/') `
                       -UpdateRegex_x64 'this is regex string' `
                       -UpdateRegex_x86 'this is also regex string'
