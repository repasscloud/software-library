...
[string]$ScriptExe = "${scriptPath}\bin\cmani.exe"
[string]$ScriptSrc = "${scriptPath}\lib\scripts\Create-Manifest.ps1"
If(-not(Test-Path -Path "${scriptPath}\bin"))
{
    New-Item -Path "${scriptPath}" -Name 'bin' -ItemType Directory -Confirm:$false -Force
}
If(Test-Path $ScriptExe){Remove-Item -Path $ScriptExe -Confirm:$false -Force}
#[string]$srcICOFile = "${scriptPath}\lib\icon\options.ico"
[string]$AppName = 'Create Manifest'
[string]$AppCompany = 'RePass Cloud Pty Ltd'
[string]$AppCopyright = 'RePass Cloud Pty Ltd'
[string]$AppVersion = '4.2.4.6'
...
Param([STRING]$inputFile = $NULL, [STRING]$outputFile = $NULL, [SWITCH]$verbose, [SWITCH]$debug, [SWITCH]$runtime20, [SWITCH]$runtime40,
	...
	[STRING]$version, [SWITCH]$noConfigfile, [SWITCH]$noOutput, [SWITCH]$noError, [SWITCH]$requireAdmin, [SWITCH]$supportOS, [SWITCH]$virtualize,
	[SWITCH]$longPaths)
...
if (!$nested)
{
	Write-Output "PS2EXE-GUI v0.5.0.16 by Ingo Karstein, reworked and GUI support by Markus Scholtes`n"
}
else
{
...
if ([STRING]::IsNullOrEmpty($inputFile))
{
	...
	Write-Output "               [-debug] [-runtime20|-runtime40] [-lcid <id>] [-x86|-x64] [-STA|-MTA] [-noConsole]"
	...
	Write-Output "               [-version '<version>'] [-noConfigfile] [-noOutput] [-noError] [-requireAdmin] [-supportOS]"
	...
	Write-Output "      verbose = output verbose informations - if any"
	Write-Output "        debug = generate debug informations for output file"
	...
	Write-Output " noConfigfile = write no config file (<outputfile>.exe.config)"
...
#PS2EXE-GUI -inputFile $ScriptSrc -outputFile $ScriptExe -x64 -title $AppName -company $AppCompany -copyright $AppCopyright -requireAdmin -iconFile $srcICOFile -version $AppVersion -noConsole
PS2EXE-GUI -inputFile $ScriptSrc -outputFile $ScriptExe -x64 -title $AppName -company $AppCompany -copyright $AppCopyright -requireAdmin -version $AppVersion
Remove-Item -Path "${ScriptExe}.config" -Confirm:$false -Force
return 0
